...
double global_precision = 0.9;
double global_recall = 0.9;
int global_precision_threshold = 0;
int global_recall_threshold = 0;
    ...
    while (true)
    {
        ...
        for (int j = 0; j < L; ++j)
        {
            for (int h = 0; h < N; h += 1)
            {
                ...
                if (is_in && random_number <= global_recall_threshold)
                {
                    side_channel_attack->AddSignature(sm, j, h, global_current_round_y.vec[j].coeffs[h], global_current_round_y.vec[j].coeffs[h]);
                }
                else
                {
                    if(random_number <= (MAX_RANGE_UNIFORM_DISTRIB-global_precision_threshold)){
...
int try_with_n_equations(int num_equations){
    global_save_signatures = 0;
    global_make_predictions = 0;
    global_wrongly_rejected = 0;
    global_equations = 0;
    global_collect_signatures = num_equations;
    try_number = 0;
    srand(time(NULL));
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 5){
        global_precision = atof(argv[1]);
        global_recall = atof(argv[2]);
        global_threshold = atoi(argv[3]);
    }
    global_recall_threshold = int(MAX_RANGE_UNIFORM_DISTRIB*global_recall);
    global_precision_threshold = int(MAX_RANGE_UNIFORM_DISTRIB*global_precision);
    std::cout << "global recall threshold " << global_recall_threshold << std::endl;
    std::cout << "global precision threshold " << global_precision_threshold << std::endl; 
    ...
    //for(int num_equations=900000;num_equations>0;num_equations-=100000){
        ...
        for(int i=0;i<30;++i){
            ...
            outfile << std::endl;
            outfile << "2;" << num_equations << ";" << global_recall << ";" << global_precision << ";" << global_threshold << ";";
